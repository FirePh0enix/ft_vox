...
pub fn recordCommandBuffer(
    ...
    command_buffer.bindIndexBuffer(self.mesh.index_buffer.asVkConst().buffer, 0, self.mesh.index_type);

    command_buffer.bindVertexBuffers(0, 3, &.{
        self.mesh.vertex_buffer.asVkConst().buffer,
        self.mesh.normal_buffer.asVkConst().buffer,
        self.mesh.texture_buffer.asVkConst().buffer,
    }, &.{ 0, 0, 0 });

    command_buffer.setViewport(0, 1, @ptrCast(&vk.Viewport{
        .x = 0.0,
        .y = 0.0,
        .width = @floatFromInt(rdr().asVk().swapchain_extent.width),
    ...
    for (world.chunks.items) |*chunk| {
        command_buffer.bindVertexBuffers(3, 1, @ptrCast(&chunk.instance_buffer.asVk().buffer), &.{0});
